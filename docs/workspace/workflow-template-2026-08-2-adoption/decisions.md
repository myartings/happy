...
| D5 | What happens to Happy-owned instructions and tool configuration? | Overwrite, preserve entirely, or translate compatible upstream semantics into local authority. | Happy owns these files. Edits are reversible; silent overwrite could break sync, releases, Paper MCP, or candidate binding. | resolved | Manually translate compatible semantics into `AGENTS.md`, `.ai/project.json`, `.codex/config.toml`, and `scripts/workflow-check.py`; preserve Happy identity, branch/devtools/release rules, product commands, tracker, protected paths, risk triggers, and Paper MCP, while strengthening formal staged checks to reject every non-evidence worktree/index divergence around each command. |
...
| D9 | What is the execution topology? | Parallel writers, separate implementation worktree, or current Root serial execution. Shared workflow contracts and runtime are tightly coupled. | Current Root; topology is reversible before edits. Parallel writes raise integration risk without independent slices. | resolved | Serial current-session Root in the existing isolated Happy worktree. No writer subagent or batch is required. Independent read-only review remains required at the review gate. |
| D10 | How is conflicted PR #63 moved from stale base `f23b9d75...` to current `origin/dev`? | Merge current `dev` into the provenance-bound delivery, rewrite the published branch, or rebuild on a new branch and regenerate base-bound evidence. | The user authorized rebuilding, replacing, and merging the PR. A stale archive cannot be repaired by conflict resolution, while a new isolated branch is reversible and avoids history rewriting. | resolved | Preserve `origin/sharp-harbor` and the PR #63 branch unchanged. Rebuild on `feature/workflow-template-2026-08-2-adoption-v2` from exact `origin/dev@a269068a...`, replay only the accepted migration, regenerate all checks/review/finish/archive evidence, push normally, create the replacement PR, close #63 only after replacement verification, and normally merge the replacement PR. |
...
- Maintain one uncommitted diff and one eventual revert boundary. The user has
  authorized the isolated delivery commit, normal branch push, replacement PR,
  closure of superseded PR #63 after replacement verification, and normal
  merge of the replacement PR. Force push, branch deletion, and history
...
- Tracker: local-only exception accepted by the user; no delayed pickup,
  delegation, or tracker write is needed. One PR delivery to `dev` is explicitly
  authorized after all local gates pass.
