...
| D10 | How is the accepted migration moved from its stale delivery base to current `origin/dev`? | Open a cumulative/conflicted PR, rewrite the published branch, or rebuild on a new branch and regenerate base-bound evidence. | The user authorized an isolated PR rebuild. Rewriting or force-pushing the published backup is avoidable; stale archive evidence makes a copied terminal commit invalid. | resolved | Preserve `origin/sharp-harbor` unchanged. Rebuild on `feature/workflow-template-2026-08-2-adoption` from exact `origin/dev@f23b9d75...`, replay only the accepted migration, regenerate checks/review/finish/archive, push normally, and target `dev`. |
...
- Maintain one uncommitted diff and one eventual revert boundary. The user has
  separately authorized the isolated delivery commit, normal branch push, and
  creation of one PR targeting `dev`; merge remains unauthorized.
