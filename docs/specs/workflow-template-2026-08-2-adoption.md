...
- Reconcile Happy-owned `AGENTS.md`, `.ai/project.json`, `.codex/config.toml`,
  `CONTEXT.md`, and `scripts/validate-happy-workflow.py` manually.
...
8. Obsolete copied tests are either safely retired by accepted fingerprints or
   replaced by a bounded Happy integration seam; no stale check remains
