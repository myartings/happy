...
def candidate_worktree_divergence(
    ...
    changed = {
        value.decode("utf-8", errors="surrogateescape")
        for value in git_bytes("diff", "--name-only", "-z", "--", *paths).split(b"\0")
        if value
    }
    ...
    return sorted(changed.intersection(paths))
...
def main() -> int:
    ...
    if slug and formal_full:
        ...
        if args.staged:
            ...
            if divergent:
                raise SystemExit(
                    "formal staged check candidate paths differ from the working tree: "
                    + ", ".join(divergent)
                )
    ...
    if candidate is not None:
        ...
        if divergent:
            print(
                "staged candidate paths changed in the working tree during check: "
                + ", ".join(divergent),
                file=sys.stderr,
            )
            failed += 1
