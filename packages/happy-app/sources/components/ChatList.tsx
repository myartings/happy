...
import { ActivityIndicator, AppState, NativeScrollEvent, NativeSyntheticEvent, Platform, Pressable, View, ViewToken } from 'react-native';
...
import { AgentWorkGroupItem, DisplayItem, TextItem, ToolGroupItem, useGroupedMessages } from '@/hooks/useGroupedMessages';
...
const ChatListInternal = React.memo((props: {
    ...
    const isGroupExpanded = useCallback((group: AgentWorkGroupItem | ToolGroupItem) => (
        groupToggles.expanded.has(group.id)
        || ((group.hasPendingPermission || (autoExpandRunningGroups && group.hasRunning)) && !groupToggles.closed.has(group.id))
    ), [autoExpandRunningGroups, groupToggles]);
        ...
        const sub = AppState.addEventListener('change', (state) => {
            ...
            setGroupToggles((prev) => {
                ...
                for (const item of displayItemsRef.current) {
                    if ((item.type === 'agent-work-group' || item.type === 'tool-group')
                        && item.hasRunning && prev.expanded.has(item.id)) {
    ...
    const handleToggleGroup = useCallback((group: AgentWorkGroupItem | ToolGroupItem) => {
        ...
        setGroupToggles((prev) => {
            const wasExpanded = prev.expanded.has(group.id)
                || ((group.hasPendingPermission || (autoExpandRunningGroups && group.hasRunning)) && !prev.closed.has(group.id));
    ...
    //
    // The personal grouping projection is already newest-first, matching the
    // inverted list. Expanded work rows are inserted in that same order.
    //
    ...
    const targetMessageId = localMessageTarget?.messageId ?? props.targetMessageId;
    const targetMessageLocalId = localMessageTarget?.localId ?? props.targetMessageLocalId;
    const targetMessageCreatedAt = localMessageTarget?.createdAt ?? props.targetMessageCreatedAt;
    ...
    const targetStoreIndex = React.useMemo(() => {
        if (!targetMessageId) return -1;
        return messages.findIndex((message) => (
            message.id === targetMessageId
            || (targetMessageLocalId != null
                && 'localId' in message
                && message.localId === targetMessageLocalId
                && (targetMessageCreatedAt === undefined || message.createdAt === targetMessageCreatedAt))
        ));
    }, [messages, targetMessageCreatedAt, targetMessageId, targetMessageLocalId]);
    const matchesTargetMessage = useCallback((message: Message) => (
        message.id === targetMessageId
        || (targetMessageLocalId != null
            && 'localId' in message
            && message.localId === targetMessageLocalId)
        || (targetMessageCreatedAt !== undefined && message.createdAt === targetMessageCreatedAt)
    ), [targetMessageCreatedAt, targetMessageId, targetMessageLocalId]);
    const targetGroup = React.useMemo(() => displayItems.find((item): item is AgentWorkGroupItem | ToolGroupItem => (
        (item.type === 'agent-work-group' || item.type === 'tool-group')
        && item.messages.some(matchesTargetMessage)
    )), [displayItems, matchesTargetMessage]);
    const targetGroupExpanded = targetGroup ? isGroupExpanded(targetGroup) : true;

    React.useEffect(() => {
        if (!targetGroup || targetGroupExpanded) return;
        setGroupToggles((prev) => {
            const expanded = new Set(prev.expanded);
            const closed = new Set(prev.closed);
            expanded.add(targetGroup.id);
            closed.delete(targetGroup.id);
            return { expanded, closed };
        });
    }, [targetGroup, targetGroupExpanded]);

    // A deep link can point at a message already cached outside the initial
    // window. Extend to its turn boundary before asking the list to scroll.
    React.useEffect(() => {
        if (targetStoreIndex < windowedMessages.length || targetStoreIndex < 0) return;
        const end = windowEndForTurn(messages, targetStoreIndex + 1, props.hasMoreOlder);
        if (end > 0) setOldestRenderedId(messages[end - 1].id);
    }, [messages, props.hasMoreOlder, targetStoreIndex, windowedMessages.length]);

    const targetAction = React.useMemo(() => {
        if (!targetMessageId || targetStoreIndex >= windowedMessages.length || !targetGroupExpanded) {
            return { type: 'none' } as const;
        }
        return resolveMessageTargetAction(
            listItems.map((item) => ({
                id: item.type === 'tool-group' && item.messages.some(matchesTargetMessage)
                    ? targetMessageId
                    : item.id,
                localId: item.type === 'message' && 'localId' in item.message
    ...
    }, [listItems, matchesTargetMessage, props.hasMoreOlder, props.isLoadingOlder, targetGroupExpanded, targetMessageCreatedAt, targetMessageId, targetMessageLocalId, targetStoreIndex, windowedMessages.length]);
    ...
    const handleViewableItemsChanged = React.useRef((info: { viewableItems: ViewToken[] }) => {
        ...
        const promptId = resolveVisiblePromptId(
            listItemsRef.current as unknown as DisplayItem[],
            visibleIndices,
        );
        if (promptId) setActivePromptId((current) => current === promptId ? current : promptId);
    }).current;
    ...
    const getItemType = useCallback((item: ListItem) => {
        ...
        if (item.type === 'tool-group') return 'tool-group';
    ...
    React.useEffect(() => {
        ...
        if (targetAction.type === 'not-found') {
            handledTargetRequestRef.current = targetRequestKey;
            sync.updateVisibleSessionTailState(props.sessionId, { targetActive: false }, tailSignalSourceId);
            return;
        }
        if (targetAction.type !== 'scroll') return;

        handledTargetRequestRef.current = targetRequestKey;
        setHighlightedMessageId(targetAction.messageId);
        setBottomDockVisibility(false);
        const scrollTimer = setTimeout(() => {
            listRef.current?.scrollToIndex({ index: targetAction.index, animated: true, viewPosition: 0.5 });
            if (Platform.OS === 'web') {
                webRevealCleanupRef.current?.();
                webRevealCleanupRef.current = revealWebMessage(getMessageTargetNativeId(targetAction.messageId));
            }
            sync.updateVisibleSessionTailState(props.sessionId, { targetActive: false }, tailSignalSourceId);
        }, 50);
    ...
    }, [agentCopyResolversByMessageId, codexFirstContract.enabled, handleToggleGroup, highlightedMessageId, isGroupExpanded, props.metadata, props.sessionId]);
    ...
    const handleScroll = useCallback((e: NativeSyntheticEvent<NativeScrollEvent>) => {
        ...
        sync.updateVisibleSessionTailState(props.sessionId, {
            atLiveTail: distanceFromNewest <= LIVE_TAIL_MAX_OFFSET,
            readingOlderHistory: distanceFromNewest > LIVE_TAIL_MAX_OFFSET,
            viewportBusy: true,
        }, tailSignalSourceId);
