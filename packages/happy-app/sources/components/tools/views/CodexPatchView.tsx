        ...
        <ToolSectionView fullWidth>
            <View style={styles.editedFileGroup}>
                <View style={[styles.patchContainer, studioPresentation && {
                    ...
                    <Pressable
                        ...
                        style={({ pressed }) => [
                            ...
                            studioPresentation && {
                                ...
                                backgroundColor: studioPresentation.diff.backgroundColor,
                                borderTopColor: studioPresentation.diff.borderColor,
                    ...
                    >
                        <View style={styles.fileHeaderMain}>
                            <Octicons name="file-diff" size={16} color={studioPresentation?.diff.metadataColor ?? theme.colors.textSecondary} />
                            ...
                            {canOpen ? (
                                <Pressable
                                    accessibilityLabel={`Open full diff for ${filePath}`}
                                    accessibilityRole="button"
                                    hitSlop={8}
                                    onPress={openFullDiff}
                                >
                                    <Octicons name="chevron-right" size={14} color={studioPresentation?.diff.metadataColor ?? theme.colors.textSecondary} />
                                </Pressable>
                            ) : null}
                            <Ionicons
                                name={expanded ? 'chevron-down' : 'chevron-forward'}
                                size={14}
                                color={studioPresentation?.diff.metadataColor ?? theme.colors.textSecondary}
                            />
                        </View>
                        {movePath ? <Text style={[styles.movePath, studioPresentation && { color: studioPresentation.diff.metadataColor }]}>{movePath}</Text> : null}
                    </Pressable>
                    {expanded && displayPatch ? (
                        <ToolDiffView patch={displayPatch} fileName={fileName} />
                    ) : expanded && diffInput?.kind === 'pair' && (diffInput.oldText.length > 0 || diffInput.newText.length > 0) ? (
                        <ToolDiffView
                            oldText={diffInput.oldText}
                            newText={diffInput.newText}
                            fileName={fileName}
