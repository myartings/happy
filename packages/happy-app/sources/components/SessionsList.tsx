...
export function SessionsList({
    ...
    const data = React.useMemo<SessionListDisplayItem[] | null>(() => {
        ...
        if (normalizedQuery) {
            ...
            const searchResults: SessionListViewItem[] = [];
            sourceData.forEach((item, index) => {
                if (item.type === 'active-sessions' || item.type === 'attention-sessions') {
                    const sessions = item.sessions.filter(matches);
                    if (sessions.length > 0) searchResults.push({ ...item, sessions });
                    return;
                }
                if (item.type === 'projects-header') {
                    if (keptProjectSources.has(item.source)) searchResults.push(item);
                    return;
                }
                if (item.type === 'project') {
                    const kept = keptProjects.get(index);
                    if (kept) searchResults.push(kept);
                    return;
                }
                if (keepIndices.has(index)) searchResults.push(item);
            });
        ...
        const archivedRows = filteredData.filter((item) => (
            item.type === 'header' || item.type === 'session'
        ));
        const groupedRows = filteredData.filter((item) => (
            item.type !== 'header' && item.type !== 'session'
        ));
    ...
    }, [hasArchivedSessions, hideArchivedSessions, machines, searchQuery, sourceData]);
