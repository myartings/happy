...
export const es: TranslationStructure = {
    ...
    status: {
        ...
        running: 'en ejecución · conectado',
        idle: 'inactivo',
    ...
    settingsFeatures: {
        ...
        showSessionModelSubtitle: 'Muestra el icono y el nombre del proveedor de IA, además del nombre y la versión del modelo',
    ...
    sidebar: {
        ...
        needsAttention: ({ count }: { count: number }) => `Requiere atención (${count})`,
    },
    ...
    toolGroup: {
        ...
        failedTools: ({ count }: { count: number }) => count === 1 ? '1 herramienta fallida' : `${count} herramientas fallidas`,
    },
    ...
    tools: {
        ...
        outputDisclosure: {
            ...
            pendingPermission: 'Esperando permiso',
            exitCode: ({ code }: { code: number }) => `salida ${code}`,
            lines: ({ count }: { count: number }) => `${count} ${count === 1 ? 'línea' : 'líneas'}`,
            truncated: 'truncado',
            omitted: '… salida omitida',
        },
        taskView: {
